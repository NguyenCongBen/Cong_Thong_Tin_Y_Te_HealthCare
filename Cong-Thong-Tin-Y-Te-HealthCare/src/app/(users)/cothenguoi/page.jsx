import "../../../../public/css/user/cothenguoi.css";
export default function CoTheNguoi() {
    return (
        <>
            <main>
                <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header model-goi">
                                <h3 class="modal-title g-blue" id="exampleModalLabel">Hotline</h3>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                <div class="g-content">
                                    <ul class="g-phone-list">
                                        <li>
                                            <div class="g-contact-pair">
                                                <div class="g-col7">
                                                    <span>Vinmec Times City (HN)</span>
                                                </div>
                                                <div class="g-col5">
                                                    <strong>
                                                        <a class="g-blue g-phone" href="#">024 3974 3556</a>
                                                    </strong>
                                                </div>
                                            </div>
                                        </li>
                                        <li>
                                            <div class="g-contact-pair">
                                                <div class="g-col7">
                                                    <span>Vinmec Times City (HN)</span>
                                                </div>
                                                <div class="g-col5">
                                                    <strong>
                                                        <a class="g-blue g-phone" href="#">024 3974 3556</a>
                                                    </strong>
                                                </div>
                                            </div>
                                        </li>
                                        <li>
                                            <div class="g-contact-pair">
                                                <div class="g-col7">
                                                    <span>Vinmec Times City (HN)</span>
                                                </div>
                                                <div class="g-col5">
                                                    <strong>
                                                        <a class="g-blue g-phone" href="#">024 3974 3556</a>
                                                    </strong>
                                                </div>
                                            </div>
                                        </li>
                                        <li>
                                            <div class="g-contact-pair">
                                                <div class="g-col7">
                                                    <span>Vinmec Times City (HN)</span>
                                                </div>
                                                <div class="g-col5">
                                                    <strong>
                                                        <a class="g-blue g-phone" href="#">024 3974 3556</a>
                                                    </strong>
                                                </div>
                                            </div>
                                        </li>
                                        <li>
                                            <div class="g-contact-pair">
                                                <div class="g-col7">
                                                    <span>Vinmec Times City (HN)</span>
                                                </div>
                                                <div class="g-col5">
                                                    <strong>
                                                        <a class="g-blue g-phone" href="#">024 3974 3556</a>
                                                    </strong>
                                                </div>
                                            </div>
                                        </li>
                                        <li>
                                            <div class="g-contact-pair">
                                                <div class="g-col7">
                                                    <span>Vinmec Times City (HN)</span>
                                                </div>
                                                <div class="g-col5">
                                                    <strong>
                                                        <a class="g-blue g-phone" href="#">024 3974 3556</a>
                                                    </strong>
                                                </div>
                                            </div>
                                        </li>
                                        <li>
                                            <div class="g-contact-pair">
                                                <div class="g-col7">
                                                    <span>Vinmec Times City (HN)</span>
                                                </div>
                                                <div class="g-col5">
                                                    <strong>
                                                        <a class="g-blue g-phone" href="#">024 3974 3556</a>
                                                    </strong>
                                                </div>
                                            </div>
                                        </li>
                                        <li>
                                            <div class="g-contact-pair">
                                                <div class="g-col7">
                                                    <span>Vinmec Times City (HN)</span>
                                                </div>
                                                <div class="g-col5">
                                                    <strong>
                                                        <a class="g-blue g-phone" href="#">024 3974 3556</a>
                                                    </strong>
                                                </div>
                                            </div>
                                        </li>
                                        <li>
                                            <div class="g-contact-pair">
                                                <div class="g-col7">
                                                    <span>Vinmec Times City (HN)</span>
                                                </div>
                                                <div class="g-col5">
                                                    <strong>
                                                        <a class="g-blue g-phone" href="#">024 3974 3556</a>
                                                    </strong>
                                                </div>
                                            </div>
                                        </li>
                                    </ul>
                                    <div class="g-button-group">
                                        <div class="g-item">
                                            <div class="g-item-desc">
                                                <img src="images/img/Download_App_8f9cde90f2.png" alt="" />
                                                <div class="g-icon-main">
                                                    <span class="g-item-title">Đặt lịch qua App - MyVinmec</span>
                                                    <span class="g-item-text">Đặt lịch hẹn chủ động ngay tại nhà với bác sĩ và
                                                        ngày giờ khám</span>
                                                </div>
                                            </div>
                                            <a class="g-a" href="#">
                                                <button class="bg-blue text-white border-none m-10">
                                                    <span>
                                                        Tải ngay
                                                    </span>
                                                </button>
                                            </a>
                                        </div>
                                        <div class="g-item">
                                            <div class="g-item-desc">
                                                <img src="images/img/CSKH_b6c956f10b.png" alt="" />
                                                <div class="g-icon-main">
                                                    <span class="g-item-title">Cổng dịch vụ CSKH 24/7</span>
                                                </div>
                                            </div>
                                            <a class="g-a" href="#">
                                                <button class="bg-blue text-white border-none m-10">
                                                    <span>
                                                        Gửi yêu cầu
                                                    </span>
                                                </button>
                                            </a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="container-all">
                    <div class="container-body">
                        <div class="container-1">
                            <div class="tt-bread">
                                <a href="#" class="tt-item">Trang chủ</a>
                                <i class="fa-solid fa-angle-right tt-item gt-item"></i>
                                <a href="#" class="tt-item tt-item-1 tt-item-color">Cơ thể người</a>
                            </div>
                            <div class="tc-bg_white tc-mb3">
                                <div class="tc-flex tc-align-item-center">
                                    <div class="tc-f26 tc-bold">
                                        Tra cứu cơ thể người
                                    </div>
                                    <div class="tc-search_drug tc-flex-one">
                                        <div class="tc-flex tc-align-item-center tc-justify-betwee">
                                            <input type="text" class="tc-txt_search_drug"
                                                placeholder="Nhập tên bộ phận cần tìm..." />
                                            <button class="tc-btn_search_drug">
                                                <img src="images/img/tra cứu/search_icon_drug.png" alt="" />
                                            </button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="tc-pb60">
                                <div class="tc-bg_white tc-mb60">
                                    <h2 class="sm-title_cate_news">Danh sách bộ phận cơ thể người</h2>
                                    <div class="tc-list_az">
                                        <div class="collapse" id="collapseExample">
                                            <div class="tc-letter-all">
                                                <p class="d-inline-flex gap-1">
                                                    <a class="btn btn-primary" data-bs-toggle="collapse"
                                                        href="#multiCollapseExample1" role="button" aria-expanded="false"
                                                        aria-controls="multiCollapseExample1" id="tc-letter">A</a>
                                                </p>
                                                <p class="d-inline-flex gap-1">
                                                    <a class="btn btn-primary" data-bs-toggle="collapse"
                                                        href="#multiCollapseExample1" role="button" aria-expanded="false"
                                                        aria-controls="multiCollapseExample1" id="tc-letter">B</a>
                                                </p>
                                                <p class="d-inline-flex gap-1">
                                                    <a class="btn btn-primary" data-bs-toggle="collapse"
                                                        href="#multiCollapseExample1" role="button" aria-expanded="false"
                                                        aria-controls="multiCollapseExample1" id="tc-letter">C</a>
                                                </p>
                                                <p class="d-inline-flex gap-1">
                                                    <a class="btn btn-primary" data-bs-toggle="collapse"
                                                        href="#multiCollapseExample1" role="button" aria-expanded="false"
                                                        aria-controls="multiCollapseExample1" id="tc-letter">D</a>
                                                </p>
                                                <p class="d-inline-flex gap-1">
                                                    <a class="btn btn-primary" data-bs-toggle="collapse"
                                                        href="#multiCollapseExample1" role="button" aria-expanded="false"
                                                        aria-controls="multiCollapseExample1" id="tc-letter">E</a>
                                                </p>
                                                <p class="d-inline-flex gap-1">
                                                    <a class="btn btn-primary" data-bs-toggle="collapse"
                                                        href="#multiCollapseExample1" role="button" aria-expanded="false"
                                                        aria-controls="multiCollapseExample1" id="tc-letter">F</a>
                                                </p>
                                                <p class="d-inline-flex gap-1">
                                                    <a class="btn btn-primary" data-bs-toggle="collapse"
                                                        href="#multiCollapseExample1" role="button" aria-expanded="false"
                                                        aria-controls="multiCollapseExample1" id="tc-letter">G</a>
                                                </p>
                                                <p class="d-inline-flex gap-1">
                                                    <a class="btn btn-primary" data-bs-toggle="collapse"
                                                        href="#multiCollapseExample1" role="button" aria-expanded="false"
                                                        aria-controls="multiCollapseExample1" id="tc-letter">H</a>
                                                </p>
                                                <p class="d-inline-flex gap-1">
                                                    <a class="btn btn-primary" data-bs-toggle="collapse"
                                                        href="#multiCollapseExample1" role="button" aria-expanded="false"
                                                        aria-controls="multiCollapseExample1" id="tc-letter">I</a>
                                                </p>
                                                <p class="d-inline-flex gap-1">
                                                    <a class="btn btn-primary" data-bs-toggle="collapse"
                                                        href="#multiCollapseExample1" role="button" aria-expanded="false"
                                                        aria-controls="multiCollapseExample1" id="tc-letter">J</a>
                                                </p>
                                                <p class="d-inline-flex gap-1">
                                                    <a class="btn btn-primary" data-bs-toggle="collapse"
                                                        href="#multiCollapseExample1" role="button" aria-expanded="false"
                                                        aria-controls="multiCollapseExample1" id="tc-letter">K</a>
                                                </p>
                                                <p class="d-inline-flex gap-1">
                                                    <a class="btn btn-primary" data-bs-toggle="collapse"
                                                        href="#multiCollapseExample1" role="button" aria-expanded="false"
                                                        aria-controls="multiCollapseExample1" id="tc-letter">L</a>
                                                </p>
                                                <p class="d-inline-flex gap-1">
                                                    <a class="btn btn-primary" data-bs-toggle="collapse"
                                                        href="#multiCollapseExample1" role="button" aria-expanded="false"
                                                        aria-controls="multiCollapseExample1" id="tc-letter">M</a>
                                                </p>
                                                <p class="d-inline-flex gap-1">
                                                    <a class="btn btn-primary" data-bs-toggle="collapse"
                                                        href="#multiCollapseExample1" role="button" aria-expanded="false"
                                                        aria-controls="multiCollapseExample1" id="tc-letter">N</a>
                                                </p>
                                                <p class="d-inline-flex gap-1">
                                                    <a class="btn btn-primary" data-bs-toggle="collapse"
                                                        href="#multiCollapseExample1" role="button" aria-expanded="false"
                                                        aria-controls="multiCollapseExample1" id="tc-letter">O</a>
                                                </p>
                                                <p class="d-inline-flex gap-1">
                                                    <a class="btn btn-primary" data-bs-toggle="collapse"
                                                        href="#multiCollapseExample1" role="button" aria-expanded="false"
                                                        aria-controls="multiCollapseExample1" id="tc-letter">P</a>
                                                </p>
                                                <p class="d-inline-flex gap-1">
                                                    <a class="btn btn-primary" data-bs-toggle="collapse"
                                                        href="#multiCollapseExample1" role="button" aria-expanded="false"
                                                        aria-controls="multiCollapseExample1" id="tc-letter">Q</a>
                                                </p>
                                                <p class="d-inline-flex gap-1">
                                                    <a class="btn btn-primary" data-bs-toggle="collapse"
                                                        href="#multiCollapseExample1" role="button" aria-expanded="false"
                                                        aria-controls="multiCollapseExample1" id="tc-letter">R</a>
                                                </p>
                                                <p class="d-inline-flex gap-1">
                                                    <a class="btn btn-primary" data-bs-toggle="collapse"
                                                        href="#multiCollapseExample1" role="button" aria-expanded="false"
                                                        aria-controls="multiCollapseExample1" id="tc-letter">S</a>
                                                </p>
                                                <p class="d-inline-flex gap-1">
                                                    <a class="btn btn-primary" data-bs-toggle="collapse"
                                                        href="#multiCollapseExample1" role="button" aria-expanded="false"
                                                        aria-controls="multiCollapseExample1" id="tc-letter">T
                                                    </a>
                                                </p>
                                                <p class="d-inline-flex gap-1">
                                                    <a class="btn btn-primary" data-bs-toggle="collapse"
                                                        href="#multiCollapseExample1" role="button" aria-expanded="false"
                                                        aria-controls="multiCollapseExample1" id="tc-letter">U
                                                    </a>
                                                </p>
                                                <p class="d-inline-flex gap-1">
                                                    <a class="btn btn-primary" data-bs-toggle="collapse"
                                                        href="#multiCollapseExample1" role="button" aria-expanded="false"
                                                        aria-controls="multiCollapseExample1" id="tc-letter">V
                                                    </a>
                                                </p>
                                                <p class="d-inline-flex gap-1">
                                                    <a class="btn btn-primary" data-bs-toggle="collapse"
                                                        href="#multiCollapseExample1" role="button" aria-expanded="false"
                                                        aria-controls="multiCollapseExample1" id="tc-letter">W
                                                    </a>
                                                </p>
                                                <p class="d-inline-flex gap-1">
                                                    <a class="btn btn-primary" data-bs-toggle="collapse"
                                                        href="#multiCollapseExample1" role="button" aria-expanded="false"
                                                        aria-controls="multiCollapseExample1" id="tc-letter">X
                                                    </a>
                                                </p>
                                                <p class="d-inline-flex gap-1">
                                                    <a class="btn btn-primary" data-bs-toggle="collapse"
                                                        href="#multiCollapseExample1" role="button" aria-expanded="false"
                                                        aria-controls="multiCollapseExample1" id="tc-letter">Y
                                                    </a>
                                                </p>
                                                <p class="d-inline-flex gap-1">
                                                    <a class="btn btn-primary" data-bs-toggle="collapse"
                                                        href="#multiCollapseExample1" role="button" aria-expanded="false"
                                                        aria-controls="multiCollapseExample1" id="tc-letter">Z
                                                    </a>
                                                </p>
                                            </div>
                                        </div>
                                        <p class="d-inline-flex gap-1">
                                            <button class="btn btn-primary" type="button" data-bs-toggle="collapse"
                                                data-bs-target="#collapseExample" aria-expanded="false"
                                                aria-controls="collapseExample">
                                                Xem
                                            </button>
                                        </p>
                                        <div class="row">
                                            <div class="col">
                                                <div class="collapse multi-collapse" id="multiCollapseExample1">
                                                    <div class="card card-body" id="ct-card-body">
                                                        <ul class="tc-list_drug tc-flex tc-first-hide">
                                                            <li>
                                                                <div class="tc-flex tc-align-item-center">
                                                                    <a class="tc-name_drug" href="#">Canxi là gì? Vai trò của
                                                                        canxi
                                                                        với cơ thể?</a>
                                                                </div>
                                                            </li>
                                                            <li>
                                                                <div class="tc-flex tc-align-item-center">
                                                                    <a class="tc-name_drug" href="#">Cao răng là gì?</a>
                                                                </div>
                                                            </li>
                                                            <li>
                                                                <div class="tc-flex tc-align-item-center">
                                                                    <a class="tc-name_drug" href="#">Cấu tạo bàn tay</a>
                                                                </div>
                                                            </li>
                                                            <li>
                                                                <div class="tc-flex tc-align-item-center">
                                                                    <a class="tc-name_drug" href="#">Cấu tạo của hậu môn</a>
                                                                </div>
                                                            </li>
                                                            <li>
                                                                <div class="tc-flex tc-align-item-center">
                                                                    <a class="tc-name_drug" href="#">Cấu tạo của Khoang
                                                                        miệng</a>
                                                                </div>
                                                            </li>
                                                            <li>
                                                                <div class="tc-flex tc-align-item-center">
                                                                    <a class="tc-name_drug" href="#">Cấu tạo của tai người - vì
                                                                        sao
                                                                        tai nghe được âm thanh?</a>
                                                                </div>
                                                            </li>
                                                            <li>
                                                                <div class="tc-flex tc-align-item-center">
                                                                    <a class="tc-name_drug" href="#">Cấu tạo của vú có gì đặc
                                                                        biệt?</a>
                                                                </div>
                                                            </li>
                                                            <li>
                                                                <div class="tc-flex tc-align-item-center">
                                                                    <a class="tc-name_drug" href="#">Cấu tạo đầu gối</a>
                                                                </div>
                                                            </li>
                                                            <li>
                                                                <div class="tc-flex tc-align-item-center">
                                                                    <a class="tc-name_drug" href="#">Cấu tạo dương vật và cơ chế
                                                                        cương dương</a>
                                                                </div>
                                                            </li>
                                                            <li>
                                                                <div class="tc-flex tc-align-item-center">
                                                                    <a class="tc-name_drug" href="#">Cấu tạo Khớp gối và các
                                                                        bệnh
                                                                        thường gặp ở khớp gối</a>
                                                                </div>
                                                            </li>
                                                            <li>
                                                                <div class="tc-flex tc-align-item-center">
                                                                    <a class="tc-name_drug" href="#">Cấu tạo khuỷu tay</a>
                                                                </div>
                                                            </li>
                                                            <li>
                                                                <div class="tc-flex tc-align-item-center">
                                                                    <a class="tc-name_drug" href="#">Cấu tạo và các bộ phận của
                                                                        mắt</a>
                                                                </div>
                                                            </li>
                                                            <li>
                                                                <div class="tc-flex tc-align-item-center">
                                                                    <a class="tc-name_drug" href="#">Cấu tạo và chức năng của
                                                                        bìu</a>
                                                                </div>
                                                            </li>
                                                            <li>
                                                                <div class="tc-flex tc-align-item-center">
                                                                    <a class="tc-name_drug" href="#">Cấu tạo và chức năng của ốc
                                                                        tai</a>
                                                                </div>
                                                            </li>
                                                            <li>
                                                                <div class="tc-flex tc-align-item-center">
                                                                    <a class="tc-name_drug" href="#">Cấu tạo và chức năng của
                                                                        ruột
                                                                        già</a>
                                                                </div>
                                                            </li>
                                                            <li>
                                                                <div class="tc-flex tc-align-item-center">
                                                                    <a class="tc-name_drug" href="#">Cấu tạo và chức năng của
                                                                        ruột
                                                                        non</a>
                                                                </div>
                                                            </li>
                                                            <li>
                                                                <div class="tc-flex tc-align-item-center">
                                                                    <a class="tc-name_drug" href="#">Cấu tạo, cách hoạt động của
                                                                        bàn
                                                                        chân</a>
                                                                </div>
                                                            </li>
                                                            <li>
                                                                <div class="tc-flex tc-align-item-center">
                                                                    <a class="tc-name_drug" href="#">Cấu tạo, trọng lượng tinh
                                                                        hoàn
                                                                        & cơ chế sinh tinh</a>
                                                                </div>
                                                            </li>
                                                            <li>
                                                                <div class="tc-flex tc-align-item-center">
                                                                    <a class="tc-name_drug" href="#">Cấu trúc và chức năng của
                                                                        bộ
                                                                        não</a>
                                                                </div>
                                                            </li>
                                                            <li>
                                                                <div class="tc-flex tc-align-item-center">
                                                                    <a class="tc-name_drug" href="#">Chân răng có gì đặc
                                                                        biệt?</a>
                                                                </div>
                                                            </li>
                                                            <li>
                                                                <div class="tc-flex tc-align-item-center">
                                                                    <a class="tc-name_drug" href="#">Chất điện giải là gì? Vai
                                                                        trò
                                                                        và cách bổ sung chất điện giải</a>
                                                                </div>
                                                            </li>
                                                            <li>
                                                                <div class="tc-flex tc-align-item-center">
                                                                    <a class="tc-name_drug" href="#">Chức năng của đĩa đệm</a>
                                                                </div>
                                                            </li>
                                                            <li>
                                                                <div class="tc-flex tc-align-item-center">
                                                                    <a class="tc-name_drug" href="#">Chức năng của hồng cầu</a>
                                                                </div>
                                                            </li>
                                                            <li>
                                                                <div class="tc-flex tc-align-item-center">
                                                                    <a class="tc-name_drug" href="#">Chức năng của tiểu cầu</a>
                                                                </div>
                                                            </li>
                                                        </ul>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="tt-phantrang">
                                            <a href="#" class="tt-item-paging active">1</a>
                                            <a href="#" class="tt-item-paging active-1">2</a>
                                            <a href="#" class="tt-item-paging active-1">3</a>
                                            <a href="#" class="tt-item-paging active-1">4</a>
                                            <a href="#" class="tt-item-paging">
                                                <img src="images/img/thông tin/product/icon-1.png" alt="" />
                                            </a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </main>
        </>
    )
}