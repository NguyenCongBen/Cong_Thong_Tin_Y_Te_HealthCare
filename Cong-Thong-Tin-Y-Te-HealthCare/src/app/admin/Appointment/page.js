...
export default function Appointments() {
    const { data, error, mutate } = useSWR('http://localhost:3000/lichhen', fetcher);
    ...
    const handleStatusChange = async (id, currentStatus) => {
        const newStatus = currentStatus === 'Đang chờ' ? 'Đã xác nhận' : 'Đang chờ'; // Ví dụ: chuyển đổi giữa 'Đang chờ' và 'Đã xác nhận'
        
        try {
            const response = await fetch(`http://localhost:3000/lichhen/${id}/status`, {
                method: 'PUT',
                headers: {
