  ...
  req.db.query(query, [hospitalId], (error, results) => {
    ...
    if (results.length === 0) {
      return res
        .status(404)
        .json({
          message: `No information found for hospital with ID ${hospitalId}.`,
        });
    }
...
router.get("/thong-tin-benh-vien/:id", (req, res) => {
  ...
  req.db.query(
    "SELECT * FROM ThongTinBenhVien WHERE id_benh_vien = ?",
    [id],
    (error, results) => {
      if (error) {
...
router.post("/thong-tin-benh-vien", (req, res) => {
  const { ten_benh_vien, mo_ta, dia_chi, so_dien_thoai, email, website } =
    req.body;
  req.db.query(
    `
        ...
        VALUES (?, ?, ?, ?, ?, ?)
    `,
    [ten_benh_vien, mo_ta, dia_chi, so_dien_thoai, email, website],
    (error, results) => {
      if (error) return res.status(500).json({ error: error.message });
      res
        .status(201)
        .json({
          id_benh_vien: results.insertId,
          message: "Bệnh viện đã được thêm",
        });
    }
  );
});
...
router.put("/thong-tin-benh-vien/:id", (req, res) => {
  ...
  req.db.query(
    `
        ...
        WHERE id_benh_vien = ?
    `,
    [ten_benh_vien, mo_ta, dia_chi, so_dien_thoai, email, website, id],
    (error, results) => {
      if (error) return res.status(500).json({ error: error.message });
      if (results.affectedRows === 0)
        return res.status(404).json({ error: "Bệnh viện không tìm thấy" });
...
router.delete("/thong-tin-benh-vien/:id", (req, res) => {
  const { id } = req.params;
  req.db.query(
    `DELETE FROM ThongTinBenhVien WHERE id_benh_vien = ?`,
    [id],
    (error, results) => {
      if (error) return res.status(500).json({ error: error.message });
      if (results.affectedRows === 0)
