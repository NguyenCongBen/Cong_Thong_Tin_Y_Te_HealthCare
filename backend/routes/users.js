...
router.put("/status/:userId", function (req, res, next) {
  ...
  req.db.query(
    "UPDATE nguoidung SET status = ? WHERE id = ?",
    [status, userId],
...
//         return res
//           .status(500)
//           .json({ message: "User already exists or an error occurred" });
...
router.post("/register", (req, res) => {
  ...
  const checkUserQuery = "SELECT * FROM nguoi_dung WHERE email = ?";

  req.db.query(checkUserQuery, [email], (err, result) => {
    if (err) {
    ...
    req.db.query(
      insertQuery,
      [ten, email, hashedPassword, vai_tro, trang_thai],
      (err, result) => {
        if (err) {
    ...
    `,
      ...
      transporter.sendMail(mailOptions, (error) => {
        if (error)
          return res
            .status(500)
            .json({ message: "Đã xảy ra lỗi khi gửi email" });
        res
          .status(200)
          .json({ message: "Email đã được gửi. Kiểm tra hộp thư của bạn." });
      });
    }
  );
});
...
router.post("/reset-password/:token", (req, res) => {
  ...
  if (!tokenData || tokenData.expiresAt < Date.now()) {
    return res
      .status(400)
      .json({ message: "Mã th ông báo không hợp lệ hoặc đã hết hạn" });
  }
